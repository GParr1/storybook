...
const InputSelect: React.FC<InputSelectProps> = ({
  ...
  const errorMessageId = fieldError ? `${selectId}-error` : undefined

  const helperMessageId = helperText ? `${selectId}-helper` : undefined
  ...
  return (
    <Container width="100%" gap="$2" opacity={disabled ? 0.6 : 1}>
      {label && (
        <Label htmlFor={selectId} color="$primaryText" fontWeight="600">
          {label}
        </Label>
      )}

      <Select {...props} id={selectId} onValueChange={handleValueChange}>
        <Select.Trigger
          width="100%"
          minHeight={44}
          paddingHorizontal="$3"
